...

{
    ...
    public function personalInfo(Request $request)
    {
        ...
        if(!isset($member_id)){ return $this->failed('认证失败,用户账号不存在',400);}
	$result=BbcMember::where('member_id',$member_id)->get();
	if(isset($result) &&  (!empty($result))){ return $this->failed('认证失败,已实名认证过',400);}
